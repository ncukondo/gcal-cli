...
describe("filterByStatus", () => {
  ...
  it("confirmed:true produces the same result as default (no options)", () => {
    const withConfirmed = filterByStatus(statusEvents, { confirmed: true });
    const withDefault = filterByStatus(statusEvents, {});
    expect(withConfirmed).toEqual(withDefault);
  });
});
